...
pub fn gen_merkle_proof(leaves: Vec<String>, leaf_pos: usize) -> Vec<Hash32Bytes> {
    ...
    for level in 0..height {
        ...
        let mut new_state: Vec<Hash32Bytes> = vec![];

        let mut left_leaf_pos = 0;
        while left_leaf_pos + 2 <= state.len() {
            let parent_hash = hash_internal(state[left_leaf_pos], state[left_leaf_pos + 1]);
